 ...
 * Purify the user input that is received through the /save POST 
 * request so that you strip all html tags from the content and clear the security risks in them and 
 * print out just the plain text "John Doe" when visiting http://127.0.0.1:8080/get-name
 */
...
app.post('/save',(request,reply)=>{
    ...
    userInput.firstname = body.firstname.replace(/(<([^>]+)>)/gi, "");
    userInput.lastname = body.lastname.replace(regexForStripHTML, '');
