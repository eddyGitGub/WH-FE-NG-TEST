 ...
 * Then you should create a write stream to a file called output.txt and 
...
const file = '/./npm-tests/output.txt'
// write code here 
let total = 0;
let values = [];

const vals = json.data.split(",")
for (var i = 0; i < vals.length; i++) {
    const obj =vals[i].split("=")
    const num = parseInt(obj[1])
    if ( num == 32){
        total++;
    }
    values.push(vals[i].replace("=", ":"))
}
...
fs.outputFile(file, values.join())
